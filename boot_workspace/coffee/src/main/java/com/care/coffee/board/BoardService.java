...
public class BoardService {
	...
	public String boardWriteProc(MultipartHttpServletRequest multi) {
		...
		if (sessionId == null)
			return "redirect:localhost/login";
...
//				f.mkdir();
	...
	public BoardDTO boardContent(String no) {
		...
		if (board != null) {
			mapper.incrementReadcount(n);
			board.setHits(board.getReadcount() + 1);

			if (board.getPicture() != null) {
				String[] names = board.getPicture().split("/");
	...
	public String boardModifyProc(BoardDTO board) {
		...
		String sessionId = (String) session.getAttribute("userId");
		if (check.getWriter().equals(sessionId) == false)
	...
	public String boardDeleteProc(String no) {
		...
		// 로그인한 아이디와 작성자 아이디가 같은지 확인
		String sessionId = (String) session.getAttribute("userId");
		if (board.getWriter().equals(sessionId) == false)
			return "작성자만 삭제 할 수 있습니다.";

		String fullPath = board.getPicture();
