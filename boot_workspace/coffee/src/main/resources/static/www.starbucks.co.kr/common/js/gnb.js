$(document).ready(function() {
    ...
    if (!(typeof eFrequencyPlannerYn === 'undefined') && eFrequencyPlannerYn == 'Y') {
        gnb +=
            '								<li><a href="/whats_new/eFreq_gift.do">2023 윈터 e-프리퀀시</a></li>'; //231017 메뉴명변경
    }
    gnb +=
        '							</ul>' +
        '							<ul>' +
    ...
    $("#totalSearch").unbind("keydown").keydown(function(e) {
        if (e.keyCode == 13) {
            e.preventDefault();
            if (pattern_check("#totalSearch", "검색어를  입력하세요.", "허용되지 않은 문자입니다.", getPattern('BASIC3')) == false) {
                return;
            }
    ...
    if (myWindow > 1100) {
        $('.gnb_sub_inner ul:nth-of-type(6)').css({
            'padding-top': '30px'
        });
        $('.gnb_sub_inner ul:nth-of-type(7)').css({
            'padding-top': '30px'
        });
        $('.gnb_sub_inner ul:nth-of-type(8)').css({
            'padding-top': '30px'
        });
    } else if (myWindow <= 1100 && myWindow > 960) {
        $('.gnb_sub_inner ul:nth-of-type(5)').css({
            'padding-top': '30px'
        });
        $('.gnb_sub_inner ul:nth-of-type(6)').css({
            'padding-top': '30px'
