...
/**
 * desc : 전체카테고리 트리구조 출력 및 배너추가
 * auth : esujin morenvy.com
 * date : 2017-04-17
 * copyright @morenvy.com : 저작권있음
 */


$(document).ready(function(){

	// 전체카테고리 html을 만들어서 리턴한다.
    function makeAllHTML( aData, parent_no, depth, classname ) {
		...
		for (index in aData){
			if( aData[index]["parent_cate_no"] == parent_no ) {
				HTML = HTML + "\n<li cate='"+aData[index]["cate_no"]+"'    value='"+aData[index]["cate_no"]+"'   ><a href='"+aData[index]["link_product_list"]+"' "+classname+" >" + aData[index]["name"] +"</a>"; 
				cate_no = aData[index]["cate_no"];                                                                       
				delete aData[index];
				if (aData.length > 0) subHTML = makeAllHTML( aData, cate_no, depth, classname );

				if ( subHTML != "" ) subHTML = "\n<div class='sub uldepth"+depth+"' ><ul class='depth"+depth+"' >"+subHTML+"\n</ul></div><div class='sub_cate_on_arrow'></div><div class='cate_arrow'></div>"; 
 

				HTML = HTML + subHTML + "</li>";
			}
		}
		return HTML;
	}
 	
	//addBanner : 첫번째 인자의 class에 두번째 인자의 (배너)내용을 넣어준다.
	function addBanner( cateSelector,  bannerSelector  ) {
		var com_HTML = "";
		var banObj = new Object();
		$(bannerSelector + " li").each(function( index ) {
			banObj[$( this ).attr("cateno")] = $( this ).html();

			var findLI = $(cateSelector + " li[cate='"+$( this ).attr("cateno")+"']");
			var findDIV = $(cateSelector + " li[cate='"+$( this ).attr("cateno")+"']").find(">div");

            if (findLI.length > 0) { // 배너가 들어갈(해당하는) li 분류가 있는 경우
                 
                //배너영역 추가 가로폭 계산
                var addedW = 0;
                var addedH = 0;
                addedW = $( this ).find(".banarea").outerWidth();
                addedH = $( this ).find(".banarea").outerHeight();
                
                 if (findDIV.length > 0) { // ul중분류가 존재한다면 끝에 붙여넣기
                    //원래 중분류 가로폭 계산
                    var orginalW = findDIV.outerWidth();
                    var orginalH = findDIV.outerHeight();
                    
                    //console.log( findDIV.css("width"));
                    findDIV.css("width",orginalW + addedW);
                    findDIV.css("height", Math.max(orginalH,  addedH));
                    findDIV.append($( this ).html());
                 } else { // ul 중분류가 없다면 새로 만들어 넣어주기
                    var uldep = findLI.parent("ul").attr("class");
                    uldep = parseInt(uldep.replace("depth", ""));
                    findLI.append("<div class='sub uldepth"+(uldep+1)+"' >"+$( this ).html()+"</ul>");

                    findLI.find(".sub").css("width", addedW);
                    findLI.find(".sub").css("height", addedH);
                  }
			}
 		});
    ...
    //원하는 depth까지만 남겨두고 삭제한다. 함수자체를 사용하지 않으면 전체카테고리로 출력한다.
	function removeUnderDepth( htmlSelector,  depth  ) {
        $(htmlSelector).find(".uldepth"+(depth+1)).remove();
        return $(htmlSelector).html();
    }
        ...
        get: function()
        {
             $.ajax({
                ...
                success: function(aData) {
                    ...
                    // 전체카테고리 html을 만들어서 보관한다.
					var morenvyAllHTML = "<ul class='depth1'>"+makeAllHTML( aData, 1, 1, " " )+"</ul>";
					$(".morenvyAllCategory").html( morenvyAllHTML ); 
                    
                    // ★ 상단메뉴 여기서부터
                    //표현할 class에 html을 넣고
					$(".category_list_box").html( morenvyAllHTML ); 
                    //addBanner : 첫번째 인자의 class에 두번째 인자의 내용을 넣어준다.
					addBanner(".category_list_box", ".openCateBannerArea");
                    //원하는 depth까지만 남겨두고 삭제한다. 함수자체를 사용하지 않으면 전체카테고리로 출력한다.
                    removeUnderDepth( ".category_list_box", 5 );
                    
                    // 마우스 오버 아웃
                    $('.category_list_box ul > li').mouseenter(function(e) {
						$(this).find(">div.sub").css("opacity","1");
                        $(this).find(">div.sub").css("visibility","visible");
						$("> a", this).addClass('sub_mouse_hover');
                    }).mouseleave(function(e) {
						$(this).find(">div.sub")
							.css("opacity","0");
						$(this).find(">div.sub").css("visibility","hidden");
						$("> a", this).removeClass('sub_mouse_hover');
                    });   

                    $('.category_list_box .depth1 > li').mouseenter(function(e) {
						$(this).find(".uldepth2").css("top","82px");
						$(this).find(">div.cate_arrow").css("top","76px");
						$(this).find(">div.cate_arrow").css("opacity","1");
                        $(this).find(">div.cate_arrow").css("visibility","visible");
                    }).mouseleave(function(e) {
						$(this).find(".uldepth2").css("top","102px");
						$(this).find(">div.cate_arrow").css("top","96px");
						$(this).find(">div.cate_arrow").css("opacity","0");
						$(this).find(">div.cate_arrow").css("visibility","hidden");
                    });   

                    $('.morenvyAllCategory .depth2 > li , .morenvyAllCategory .depth3 > li').mouseenter(function(e) {
                        $(this).find(">div.sub").css("display","inline");
						$("> a", this).addClass('sub_mouse_hover');
                    }).mouseleave(function(e) {
					...
					$('.category_list_box .depth2 > li , .morenvyAllCategory .depth2 > li').mouseenter(function(e) {
						$(this).find('> .sub_cate_on_arrow').addClass('rotateon');
					}).mouseleave(function(e) {
						$(this).find('> .sub_cate_on_arrow').removeClass('rotateon');
					});

					/* morenvy.com 상단 카테고리 하위분류 */
					$('.category_list_box .depth3 > li , .morenvyAllCategory .depth3 > li').mouseenter(function(e) {
						$(this).find('> .sub_cate_on_arrow').addClass('rotateon');
					}).mouseleave(function(e) {
						$(this).find('> .sub_cate_on_arrow').removeClass('rotateon');
					});
                    // morenvyAllHTML을 이용하여 전체카테고리 또는 왼쪽메뉴 카테고리를 추가로 더 만들어 사용할 수 있다.
                }
...
$(document).ready(function(){
    ...
    var methods = {
        ...
        show: function(overNode, iCateNo) {
            ...
            if (methods.aSubCategory[iCateNo] != undefined) {
                aHtml.push('<ul class="slideSubMenu"><div class="ul_inner">');
                $(methods.aSubCategory[iCateNo]).each(function() {
                    ...
                    aHtml.push('<a href="'+this.link_product_list+'" class="cate" cate="'+this.param+'" onmouseover="subMenuon(this);">'+this.name+'</a>'); // 중분류
                    ...
                    if (methods.aSubCategory[sNextParentNo] != undefined) {
                        ...
                        $(methods.aSubCategory[sNextParentNo]).each(function() {
                            ...
                            aHtml.push('<a href="'+this.link_product_list+'" class="cate" cate="'+this.param+'" onmouseover="subMenuon(this);">'+this.name+'</a>'); // 소분류
                            ...
                            if (methods.aSubCategory[sNextParentNo2] != undefined) {
                                ...
                                $(methods.aSubCategory[sNextParentNo2]).each(function() {
                                    aHtml.push('<li class="noChild" id="cate'+this.cate_no+'">');
                                    var sCateSelected = (checkInArray(sMyCateList, this.cate_no) == true) ? ' selected' : '';
                                    aHtml.push('<a href="'+this.link_product_list+'" class="cate" cate="'+this.param+'" onmouseover="subMenuon(this);">'+this.name+'</a>'); // 하위분류
                                    aHtml.push('</li>');
                                });
                                aHtml.push('</ul>');
                            }
        ...
        checkSub: function() {
            $('.cate').each(function(){
                ...
                if (result == undefined) {
                    var sHref = '/product/list.html'+$(this).attr('cate'); // 중분류 없는 대분류
	...
	jQuery('#mcategory ul li').mouseenter(function(e){
...
jQuery(document).ready(function() {
	...
	jQuery(function () {
		...
		if (!msie6) {
			var top = jQuery('#mheader').offset().top - parseFloat(jQuery('#mheader').css('margin-top').replace(/auto/, 0));
			jQuery(window).scroll(function (event) {
				...
				jQuery('#mheader').addClass('fixed');
					...
					jQuery('#mheader.fixed').css('margin-left',-s_left);
				...
				// otherwise remove it
				jQuery('#mheader').removeClass('fixed').css('margin-left',0);
				}
			});
		}
	});
	...
	var mh_inner = 0;
	jQuery('#mheader > .inner > div').each(function(){
		mh_inner = mh_inner + jQuery(this).width();
	})
	if( jQuery('#mheader > .inner').width() < mh_inner + 30 ){
		jQuery('#mheader').addClass('over_w')
	}
	...
	jQuery(window).resize(function(){
		...
		if( jQuery('#mheader > .inner').width() <  mh_inner + 30 ){
			...
			jQuery('#mheader').removeClass('over_w')
		}
	})


	/* 상단 검색 */
	jQuery(".top_sec .search_btn").click(function(){
		jQuery(".top_sec .search_icon").addClass('on');
	...
	/* 상단 검색 - 닫기 */
	jQuery(".search_under .search_close").click(function(){
		jQuery(".top_sec .search_icon").removeClass('on');
	});

	/* 상단 검색 - morenvy.com 검색창 문구 */
	jQuery( ".xans-layout-searchheader fieldset" ).each(function() {
		var searchheader_text = jQuery(this).attr('title');
		jQuery('.xans-layout-searchheader #keyword').attr('placeholder', searchheader_text)
	});
	
	/* 상단 검색 - 추천 검색어 번호 */	
	jQuery('.search_under .pop_title a').each(function(){
		var num = jQuery(this).index() + 1;
		if(num < 10){
			jQuery(this).find('.num').text('0' + num);
		} else {
			jQuery(this).find('.num').text(num);
		}
	})

	/* morenvy.com 하단 패밀리사이트 */
	$('.mf_site .family, .mf_site ul').click(function() {
		$(this).toggleClass('on');
		if($(this).hasClass('on')) {
			$('.mf_site ul').slideDown("fast");
		} else {
			$('.mf_site ul').slideUp('fast');
		}
	});
	
	/* morenvy.com 목록페이지 - 추천상품 슬라이드 */
	var list_recmd = new Swiper('.list_recmd', {
		roundLengths: true,
		observer: true,
		observeParents: true,
		slidesPerView: '3',
		...
		navigation: {
			nextEl: '.swiper-button-next-recmd',
			prevEl: '.swiper-button-prev-recmd',
        }
	});

	/* morenvy.com 목록페이지 - 신상품 슬라이드 */
	var list_new = new Swiper('.list_new', {
		roundLengths: true,
		observer: true,
		observeParents: true,
		slidesPerView: '3',
	...
	jQuery('.xans-product-listrecommend .thumbnail .num').each(function(index) {
		jQuery(this).append('BEST '+'0'+(index+1));
	});
	jQuery('.xans-product-listnew .thumbnail .num').each(function(index) {
		jQuery(this).append('NEW '+'0'+(index+1));
	});
	...
	function brand_h () {
		jQuery('.ec-base-product .prdList > li').each(function(){
			if( jQuery(this).find("[rel='브랜드']").length != 0 ){
				jQuery(this).find('.description').addClass('exist_b');	
			}
		});
	};
...
function cal_percent () {
	jQuery("[rel='판매가']").each(function(){
		...
		if(jQuery(this).siblings("[rel='할인판매가']").length != 0 && m_price != m_sale){
			...
			jQuery(this).parents('li').find('span.time_sale_p').text(p_sale + '% ' + 'OFF ↓')
			...
			if (m_custom != m_price){
				...
				jQuery(this).addClass('msale');
				jQuery(this).siblings("[rel='소비자가']").hide();
				jQuery(this).parents('li').find('.sale_box').hide();
