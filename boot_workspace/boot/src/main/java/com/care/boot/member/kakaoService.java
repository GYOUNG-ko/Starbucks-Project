...
public class kakaoService {
	...
	public void getAccessToken(String code) {
		...
		reqParam += "&client_id=a45da73b862067465ac6e9cb78fd1afa";
		reqParam += "&rediect_uri=http://localhost:8085/dbQuiz/kakaoLogin";
		reqParam += "&code="+code;
		
		try {
			URL url = new URL(reqUrl); // POST 요청에 필요로 요구하는 파라미터 스트림을 통해 전송
			HttpURLConnection conn = (HttpURLConnection) url.openConnection(); // 연결맺음
			
			conn.setRequestMethod("POST"); // POST 요청을 위해 기본값 false에서 setDoOutput을 true로
